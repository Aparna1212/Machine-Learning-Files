...
Based on https://www.analyticsinhr.com/blog/tutorial-people-analytics-r-employee-churn/

#Data Load and Prep
Load an R data frame by reading in the .csv file
If you need to figure out where your working directory is, use getwd()
Remember to put ~/ in front of the filename, which says look in the working dir.
#####For problems installing rattle or RGtk2 follow links below


```{r}
#devtools::install_github("r-lib/pillar")
#install some extra packages, NOTE:  THESE ARE ADDED

#install.packages("RGtk2")
#install.packages("rattle")
###Additional notes on installing RGtk2 and rattle
...
library(plyr)   #Call the plyr library
library(dplyr)  #Call the dplyr library
#Bring in the powerful graphics library ggplot2
library(ggplot2)
library(caret)

library(rattle)   #rattle provides a gui for other data mining packages in r
...
MYdataset <- MFG10YearTerminationData  #Name the data frame
str(MYdataset)  #Examine the structure of the data frame

#Check data quality by calling up a summary of the data
summary(MYdataset)

#initial look, notice the use of pipes in the code
#the pipes create StatusCount by grouping by STATUS_YEAR and STATUS and creating a table
...
#Create barchart of termreason by year
...
#Create barchart of reason for termination by department
...
#Classifcation Models - Decision Tree
...
MYrf <- randomForest::randomForest(STATUS ~ .,
                                   ...
                                   mtry=2,
                                   importance=TRUE,
                                   na.action=randomForest::na.roughfix, #this imputes for the NAs by median or mode
                                   replace=FALSE)
...
# The `pROC' package implements various Area Under the Curve (AUC) functions.

# Calculate the Area Under the Curve (AUC).  The higher the AUC the better.
...
# Evaluate model performance - Sensitivity/Specificity Plot: requires the ROCR package
...
#Apply model - Generate 2016 data
```{r include=TRUE}
